...
def non_py_just_eval(cards):
    mask = 0
    for card in cards:
        mask |= 1L << (map_suit_to_offset[card.suit] + map_rank_to_offset[card.rank])
    return eval7.non_py_evaluate(mask)
...
class TestEval7(unittest.TestCase):
    ...
    def test_hand_vs_range_monte_carlo(self):
        ...
        villain = HandRange("AA,A3o,32s")
        board = []
        equity = eval7.py_hand_vs_range_monte_carlo(hand, villain, board, 10000000)
