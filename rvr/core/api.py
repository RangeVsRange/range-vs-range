...
@api
def get_user_by_screenname(screenname, session):
    """
...
@api
def get_user_games(userid, session):
    """
...
def _start_game(session, open_game):
    """
...
@api
def join_game(userid, gameid, session):
    """
    ...
    if any(ogp.userid == userid for ogp in game.ogps):
        return ERR_JOIN_GAME_ALREADY_IN
    game.participants += 1
    if game.participants > game.situation.participants:
        return ERR_JOIN_GAME_GAME_FULL
    ...
    if start_game:
        session.flush()
        running_game = _start_game(session, game)
    else:
        running_game = None
    ...
    try:
        ...
        running_game = join_game(userid, gameid, session)

    logging.debug("User %d joined game %d", userid, gameid)

    ensure_open_games(session)
...
@api
def leave_game(userid, gameid, session):
    """
